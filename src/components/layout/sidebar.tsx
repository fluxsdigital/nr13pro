    ...
    <>
      ...
      <div className={cn("border-t border-sidebar-border", collapsed ? "p-2" : "p-4")}>
        {collapsed ? (
          <button
            onClick={toggle}
            className="w-full flex justify-center py-2 rounded-lg text-sidebar-foreground/40 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors"
            title="Expandir menu"
          >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              onClick={toggle}
              className="shrink-0 p-1.5 rounded-lg text-sidebar-foreground/40 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors"
              title="Recolher menu"
            >
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "hidden md:flex bg-sidebar text-sidebar-foreground flex-col h-screen fixed left-0 top-0 border-r border-sidebar-border transition-all duration-300",
          expanded ? "w-64" : "w-16"
        )}
      >
